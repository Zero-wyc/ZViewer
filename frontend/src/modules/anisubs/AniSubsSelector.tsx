...
import {
  ...
  getAniSubsEpisodes,
  type AniSubsSource,
  type AniSubsSearchResult,
  type AniSubsEpisode,
} from '@/modules/anisubs'
import { cn } from '@/lib/utils'

interface AniSubsSelectorProps {
  open: boolean
...
export function AniSubsSelector({
  open,
  onOpenChange,
  onSelectEpisode,
  disabled,
  ...
  const [searchResults, setSearchResults] = useState<AniSubsSearchResult[]>([])
  const [episodesMap, setEpisodesMap] = useState<
    Record<string, AniSubsEpisode[]>
  >({})
  ...
  const [viewMode, setViewMode] = useState<'list' | 'tile'>(() => {
    const saved = localStorage.getItem('anisubs-selector-view-mode')
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      console.error('[AniSubsSelector] search error:', err)
    } finally {
      setSearching(false)
    }
  ...
  const handleToggleExpand = useCallback(
    async (result: AniSubsSearchResult) => {
      ...
      try {
        const episodes = await getAniSubsEpisodes(result.source, result.id)
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          >
            <button
              ...
              onClick={() => {
                setViewMode('list')
                localStorage.setItem('anisubs-selector-view-mode', 'list')
              }}
            ...
            <button
              ...
              onClick={() => {
                setViewMode('tile')
                localStorage.setItem('anisubs-selector-view-mode', 'tile')
              }}
              className="flex items-center gap-1.5 rounded-[calc(var(--md-sys-shape-corner)-2px)] px-3 py-1.5 text-xs font-medium transition-all"
              style={{
                backgroundColor:
