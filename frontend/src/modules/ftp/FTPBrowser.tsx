...
export default function FTPBrowser({
  ...
  const [entries, setEntries] = useState<FTPDirectoryEntry[]>([])
  ...
  const load = useCallback(
    async (path?: string) => {
      ...
      setError('')
      try {
        const data = await browseFTPMount(mountId, path)
        setEntries(data)
        setCurrentPath(path)
      } catch (err) {
  ...
  const handleEntryClick = (entry: FTPDirectoryEntry) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="浏览 FTP 目录">
