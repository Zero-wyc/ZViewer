...
export default function WebDAVBrowser({
  ...
  const [entries, setEntries] = useState<WebDAVDirectoryEntry[]>([])
  ...
  const load = useCallback(
    async (path?: string) => {
      ...
      setError('')
      try {
        const data = await browseWebDAVMount(mountId, path)
        setEntries(data)
        setCurrentPath(path)
      } catch (err) {
  ...
  const handleEntryClick = (entry: WebDAVDirectoryEntry) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="浏览 WebDAV 目录">
